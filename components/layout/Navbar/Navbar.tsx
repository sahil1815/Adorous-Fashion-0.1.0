  ...
  return (
    ...
    >
      ...
      >
        ...
        <span
          className={`absolute -bottom-1 left-0 h-px bg-[#B76E79] transition-all duration-300 ${isActive ? "w-full" : "w-0 group-hover:w-full"}`}
        />
      </Link>
        ...
        <div className="absolute top-full left-1/2 -translate-x-1/2 mt-3 bg-white border border-[#F7E7CE] shadow-[0_8px_40px_rgba(26,26,26,0.08)] rounded-sm min-w-[180px] py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-150">
          {link.children.map((child) => (
            <Link
              key={child.href}
              href={child.href}
              className="block px-5 py-2.5 text-[12px] tracking-[0.08em] uppercase text-[#1A1A1A] hover:text-[#B76E79] hover:bg-[#F7E7CE]/40 transition-colors duration-150 whitespace-nowrap"
            >
              {child.label}
            </Link>
          ))}
    ...
    <div className="fixed inset-0 z-[100] bg-white/95 backdrop-blur-sm flex flex-col items-center justify-center animate-in fade-in duration-200">
      ...
      <form onSubmit={handleSearch} className="w-full max-w-xl px-6 relative">
        <input
          ...
          className="w-full border-0 border-b-2 border-[#1A1A1A] bg-transparent text-[#1A1A1A] text-xl tracking-wide pb-3 pr-10 outline-none focus:border-[#B76E79]"
        />
        <button
          type="submit"
          className="absolute right-8 top-1/2 -translate-y-1/2 text-[#1A1A1A]/40 hover:text-[#B76E79]"
        >
          <Search size={18} />
        </button>
      </form>
...
function MobileMenu({
  ...
  return (
    <>
      <div
        className={`fixed inset-0 z-40 bg-[#1A1A1A]/30 backdrop-blur-[2px] transition-opacity duration-300 ${isOpen ? "opacity-100" : "opacity-0 pointer-events-none"}`}
        onClick={onClose}
      />
      <nav
        className={`fixed top-0 left-0 h-full w-[80vw] max-w-sm z-50 bg-[#F7E7CE] flex flex-col transition-transform duration-400 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
            ...
            <li key={link.href}>
              {link.children ? (
                    ...
                    {link.children.map((child) => (
                      <li key={child.href}>
                        <Link
                          href={child.href}
                          onClick={onClose}
                          className={`block text-[12px] tracking-[0.12em] uppercase font-medium ${pathname.startsWith(child.href) ? "text-[#B76E79]" : "text-[#1A1A1A]"}`}
                        >
                          {child.label}
                        </Link>
                      </li>
                ...
                <Link
                  href={link.href}
                  onClick={onClose}
                  className={`block py-3 text-[13px] tracking-[0.16em] uppercase font-medium border-b border-[#1A1A1A]/10 ${pathname.startsWith(link.href) ? "text-[#B76E79]" : "text-[#1A1A1A]"}`}
                >
                  {link.label}
                </Link>
              )}
  ...
  return (
    <>
      ...
      >
        ...
        <div
          className={`max-w-[1400px] mx-auto px-4 md:px-10 flex items-center transition-all duration-500 ${scrolled ? "py-3" : "py-4 md:py-6"}`}
        >
          ...
          <div className="flex-[2] flex justify-center">
            ...
            >
              <span
                className="text-[18px] sm:text-[22px] md:text-[28px] tracking-[0.2em] md:tracking-[0.35em] font-light text-[#1A1A1A] leading-none"
                style={{ fontFamily: "'Cormorant Garamond', serif" }}
              >
                ADOROUS
              </span>
          ...
          <div className="flex-1 flex justify-end items-center gap-1 sm:gap-4 md:gap-6">
            <button
              ...
              className="p-2 text-[#1A1A1A] hover:text-[#B76E79] transition-colors"
            >
              <Search size={19} />
            </button>
            <Link
